...
def cleaning(task, ds, sketches, group_records, condition_records, valid_records, run_stats, verbose, demo, res_demo):
    ...
    while len(entities) < task.sample_size:
        ...
        if pivot_record["group"] == target_group:
            ...
            if demo:
                entity_idx = len(df_records)
                entity["_level"] = "entity"
                df_records.append(entity)
                entity_records = ds[ds["_id"].isin(pivot_record["matches"])].to_dict("records")
                for record in entity_records:
                    record["_level"] = "record"
            ...
            if demo:
                if st.session_state.df is None:
                    clean_sample = pd.DataFrame(df_records)
                    clean_sample["_id"] = range(len(clean_sample))
                    clean_sample["_entity_id"] = clean_sample["_id"].where(clean_sample["_level"] == "entity").ffill()
                    clean_sample = clean_sample.set_index(["_entity_id", "_id"])
                    st.session_state.df = clean_sample
                else:
                    new_rows = pd.DataFrame(df_records[entity_idx:])
                    clean_sample = pd.concat([clean_sample, new_rows], ignore_index=True)
                    clean_sample["_id"] = range(len(clean_sample))
                    clean_sample["_entity_id"] = clean_sample["_id"].where(clean_sample["_level"] == "entity").ffill()
                    clean_sample = clean_sample.set_index(["_entity_id", "_id"])
                    st.session_state.df = clean_sample
